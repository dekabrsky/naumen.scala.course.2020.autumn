...
object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    val res = passage(number).distinct
    res
  }

  def passage(curVal: Int, factors: List[Int] = List()): List[Int] = {
    for(i <- 2 to curVal if curVal % i == 0)
      return passage(curVal / i, factors :+ i)
    factors
  }
    ...
    def round2(i: Double): Double ={ //для удобства тестирования
        Math.round(i*100)/100.00
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
      ListMap(ballsArray.toSeq.sortWith((x, y) => getWeight(x._2._1, x._2._2) < getWeight(y._2._1, y._2._2)):_*).keys.toSeq
    }

    def getWeight(radius:Int, density: Double): Double ={
      4.00/3.00*java.lang.Math.PI*Math.pow(radius, 3)*density
    }
}